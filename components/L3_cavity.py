...
def L3_cavity(r: float = 0.2,
    ...
    phc_wvg = PhC_wvg_hex_lattice(r=r, 
                                  ...
                                  width=None, 
                                  wvg_type=1)
    component.add_ref(component=phc_wvg)
    ...
    for i in range(int((n_holes-4)/2)):
        component.add_ref(component=hole).dmove((phc_wvg.dxmax - r - (2*i+1)*(a/2), 
                                                 phc_wvg.dy))
        component.add_ref(component=hole).dmove((phc_wvg.dxmin + r + (2*i+1)*(a/2), 
                                                 phc_wvg.dy))
